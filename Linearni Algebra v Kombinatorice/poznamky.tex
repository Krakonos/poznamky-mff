...
$p_i^2$ se ztratí do $\alpha$ a $\beta$. Následuje rozbor případů:
...
$${n(n-1) \over 2} + {6n\over 2} + {4\over 2} = {n^2-5nn+4 \over 2} = {(n+1)(n+4)\over 2}$$ \qed
...
{\bf Horní odhad} (ostatní řádky nepotřebujeme):
\begin{align}
	&x_i^2 &\qquad n\\
	&x_ix_j &\qquad {n \choose 2}\\
	&x_i &\qquad n\\
...
$${n(n-1)\over 2} + {4n\over 2} = {n^2+3n\over 2} = {n(n+3) \over 2}$$
...
$$\left\{x | \sum x_i = 2 \right\} \cap \R^{n+1} \approx \R^n$$
