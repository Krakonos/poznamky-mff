...
\dk $m = 0$ nezávisí na konkrétní volbě $u, v$. Dále indukcí dle $m$.
...
$(A_i)_{uv} = 1$\quad pokud $d_G(u,v) = i$\hfil $A_1 = A$

$(A_i)_{uv} = 0$\quad pokud $d_G(u,v) \neq i$\hfil $A_0 = I$

$Z_{mi} = 0 \Leftarrow i > m$\\
$A^0 = A_0$\\
$A^1 = A_1$\\
$A^2 = A_0 + A_1 + Z_{2,2} \cdot A_2$\\
$\vdots$\\
$A^d = \dots + Z_{d,d}\cdot A_d$

Generujeme celý vektorový prostor $\Rightarrow$ $\dim \A(G) = d+1$.

\todo Idea tohohle důkazu mi nějak uniká\dots{} Nechybí v něm něco?
