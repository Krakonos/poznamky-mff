...
\setcounter{equation}{0}
\uv{Existuje alespoň jedna množina:}
\begin{align}
	(\exists x) (x = x)
\end{align}
...
\setcounter{equation}{0}
\uv{Množiny, které mají tytéž prvky, se sobě rovnají:}
\begin{align}
...
\setcounter{equation}{0}
\uv{Z každé množiny lze vydělit množinu prvků, které splňují stejné vlastnosti:} 
\\
...
\setcounter{equation}{0}
\uv{Libovolné dvě množiny určují dvouprvkovou množinu:}
\begin{align}
...
\setcounter{equation}{0}
\uv{Ke každé množině existuje množina všech prvků, které náleží do nějakého 
prvku dané množiny:}
\begin{align}
...
\setcounter{equation}{0}
\uv{Ke každé množině existuje množina sestávající ze všech jejích podmnožin:}\\
\begin{align}
...
\setcounter{equation}{0}
\uv{Definovatelné zobrazení zobrazuje množinu na množinu:}\\
Nechť $\psi(u,v)$ je formule, která neobsahuje volné proměnné $w$ a $z$. Potom 
formule
\begin{align}
%	(\forall u)(\forall v)( \forall w((\psi(u,v) \land \psi(u,w)) \impl v = w) 
%	\impl (\forall a) (\exists z)(\forall v) (v\in z \eq (\exists u) (u \in a 
%	\land \psi(u,v))))
...
\setcounter{equation}{0}
\uv{Každá neprázdná množina má alespoň jeden prvek, který s ní má prázdný 
průnik}
\begin{align}
